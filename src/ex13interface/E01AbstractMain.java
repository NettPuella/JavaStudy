...
/* 추상클래스2: 추상 메서드를 포함한 클래스. 추상메서드를 멤버로
...
부모의 추상 메서드를 자식쪽에서 보고 오바라이딩 하면된다. */
class AbsClassChild2 extends AbsClass2{
	/* 오버라이딩은 부모쪽의 메서드를 가리고, 새로운 기능을 재정의
	하는 것이므로 해당 클래스는 추상메서드를 포함하지 않은 상태가 된다.
	따라서 인스턴스 생성이 가능해진다.
	아래 오버라이딩 메서드를 삭제하면 에러가 발생한다. */
	@Override
...
public class E01AbstractMain {
	...
	public static void main(String[] args) {
		...
		/* 추상클래스내에 선언된 정적메서는 오버라이딩의 대상이
		될 수 없으므로 static의 기본규칙을 따른다. 별도의 생성
		절차없이 클래스명으로 바로 접근할 수 있다. */
