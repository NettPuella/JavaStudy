...
참조변수 타입에 따라 다영한 형태의 인스턴스를 이용ㅇ할 수 있는 특성을
말한다.
...
4. 이것을 이질화ㅣ(Heterogeneous : 헤테로지니어스)라고 표현한다.
...
class Parent{
	int parentMember;
	void parentMethod() {
		System.out.println("부모의 메소드: parntMethod()");
	}
}
class Child extends Parent{
	...
	void prentMethod(int childMember) {
...
public class E11Polymorphism {
	...
	public static void main(String[] args) {
		...
		소괄호 하나 더 추가해서 parent2를 자식타입으로 형변환한 후
		멤버변수에 접근해야한다. */
		((Child)parent2).childMember = 1;
		((Child)parent2).childMethod();
		((Child)parent2).parentMethod();
		
		/* 자식타입으로 형변환 이후에 해당 참조변수를 통해 자식
		멤버에게 접근한다. 위와 동일한 방법이다. */
		Child child2 = (Child)parent2;
		child2.childMember = 1;
		child2.childMethod();
		child2.prentMethod(1);
		...
		선언없이 사용할 수 있고, 필요하다면 오버라이딩도 할수 있다.*/
