...
public class PersonConstructor {
	...
	String addr;
	/*
	디폴트 생성자: 해당 클래스에 생성자를 정의하지 않으면 아래와 같이 매개변수, 실행부가 없는
	생성자가 컴파일러에 의해 자동으로 삽입된다. 지금까지 우리가 new 클래스명()과 같이
	사용했던 문장이 바로 디폴트 생성자를 호출하여 객체를 생성하는 것이었다. 
	*/
	//생성자 메소드
	...
	public PersonConstructor(String name) {
		...
		용도로 사용된다. 클래스 자신, 즉 멤버를 가르킨다. 따라서
	...
	public PersonConstructor(String name, int age) {
		...
		사용할 수 없다. 즉 아래코드는 매개변수가 3개인 생성자를
		호출한다.
		*/
		this(name, age, "미상");
	...
	void initialize(String name, int age, String addr) {
		...
		호출할 수 없다. 멤버메서드는 개발자가 원할때 언제든 호출이
		가능하므로 생성자와는 용도와 특성이 다르다. 따라서
		멤버메서드에서는 새엇ㅇ자를 호출할 수 없다. */
		//this(name, age, "미상"); //=> 에러발생
		
		//멤버변수 구분을 위한 this멤버서드에서도 사용할 수 있다
		this.name = name;
		this.age = age;
		this.addr = addr;
	}
