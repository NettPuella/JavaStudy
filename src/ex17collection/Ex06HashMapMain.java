...
//		map.put("age", number);
		...
		*/
		...
		/*
		 ...
		 해당 반환값을 확장 for문을 통해 반복하고, 이를 통해 Value값을
		 추출할 수 있다.
		*/
		Set<String> keys = map.keySet();
		System.out.println("[확장for문 적옹]");
		for(String key : keys) {
		...
		*/
		...
		while(it.hasNext()) {
			...
			//Key를 통해 Value를 추출한다.
			String value = map.get(key); //벨류값 억기
			System.out.println(String.format("%s:%s", key, value));
		}
		...
		*/
		...
		객체삭제 : Key를 통해 삭제하고, 삭제에 성공하면 해당 value가
			반환된다.
		*/
		System.err.println("삭제된객체:" + map.remove("age"));
