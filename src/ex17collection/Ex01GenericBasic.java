...
public class Ex01GenericBasic {
	...
	public static void main(String[] args) {
		...
		할수 없다. 컴파일 에러가 발생하므로 실행자체가 불가능하다.
		즉 자료향에는 안전하지만, 구현에는 불편함이 있다.
		다른 객체를 저장하기 위해서는 또 다른 Box클래스를 생서해야한다. */
...
//		oBox1.store("당도가 20인 오렌지");
		...
		더 어렵다. */
		//Orange 객체를 저장 후 꺼내는건 문제없이 실행된다.
		FruitBox fBox1 = new FruitBox(new Orange(20));
		Orange orange3 = (Orange)fBox1.pullOut();
		orange3.showInfo();
		
		/* 하지만 오렌지같은 String을 저장하고, 이를 꺼내서
		다운캐스팅 하는 부분에서는 예외가 발생된다. 즉 런타임
		에러가 발생하게 되므로 실행해보기 전에는 오류를 예측하기 쉽지않다.*/
